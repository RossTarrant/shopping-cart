...
export default function CartIcon({basketQuantity, switchBasketView}){
    ...
    return(
        <div className={"flex gap-2 justify-center content-center"}>
            ...
            <Link to={"/cart"}><span data-testid="basket" className={"text-xl bg-red-600 font-bold rounded-full w-8 h-8 flex items-center justify-center"}>{basketQuantity}</span></Link>
        </div>
    )
}